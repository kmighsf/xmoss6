...
const __qd_g_exXP = [''];
var __qd_g_isfullscn = false;
var __qd_g_new_playleft_id = null;
var __qd_margin_bak = '';


function __qd_playfull_set(_in_id, _in_title_on, _in_exXP){
  if (!navigator.userAgent.match(/(iPhone|iPod|Android|mobile|blackberry|webos|incognito|webmate|bada|nokia|lg|ucweb|skyfire)/i)) {
    ...
    $((".fullscn" + _in_exXP)).click(function() {
      if(!__qd_g_isfullscn){
        ...
        __qd_margin_bak = $('#'+_in_id + ' iframe').css('margin');
        $('#'+_in_id + ' iframe').css('margin', '0px');
        $('#'+_in_id).attr("id", _new_ID);

        //
        __qd_g_new_playleft_id = _new_ID;

        //
        __qd_g_isfullscn = true;
      }
      else {
        $((".fullscn" + '')).html(_in_title_on);
        $('#'+_in_id + ' iframe').css('margin', __qd_margin_bak);
        $(('#' + __qd_g_new_playleft_id)).attr("id", _in_id);

        //
        __qd_g_isfullscn = false;
      }
    });
  };
}

function __qd_exp_playfull_set(_in_id){
  for (var i = 0; i < __qd_g_exXP.length; i++){
    const p1 = (__qd_g_exXP[i] ? ('网页' + __qd_g_exXP[i] + 'P') : '网页全屏');
    const p2 = (__qd_g_exXP[i] ? ('-' + __qd_g_exXP[i] + 'p') : '');
    __qd_playfull_set(_in_id, p1, p2);
  }
}
